...
namespace IguanaClient
{
    ...
    public static partial class Example
    {
        public static void T16()
        {
            ...
            Kernel.GeoOCC.AddBox(0, 0, 0, 0.5, 0.5, 0.5, 2);

            // We apply a boolean difference to create the "cube minus one eigth" shape:
            Tuple<int, int>[] ov;
            Kernel.GeoOCC.Cut(new Tuple<int, int>[] { Tuple.Create(3, 1) }, new Tuple<int, int>[] { Tuple.Create(3, 2) }, out ov, 3);
            ...
            for (int t = 1; t <= 5; t++)
            {
                x += 0.166;
                z += 0.166;
                Kernel.GeoOCC.AddSphere(x, y, z, r, 3 + t, - Math.PI / 2, Math.PI / 2, 2 * Math.PI);
            ...
            Tuple<int, int>[][] ovv;
            Kernel.GeoOCC.Fragment(new Tuple<int, int>[] { Tuple.Create(3, 3) }, holes, out ov, out ovv);
            ...
            for (int i = 1; i <= 5; i++) Kernel.Model.AddPhysicalGroup(3, new int[] { 3 + i }, i);

            // The tag of the cube will change though, so we need to access it
            // programmatically:
            Kernel.Model.AddPhysicalGroup(3, new int[] { ov[ov.Length - 1].Item2 }, 10);
            ...
            Kernel.MeshingKernel.SetSize(ov, lcar1);

            // Override this constraint on the points of the five spheres:
            //IguanaGmsh.Model.GetBoundary(holes, out ov, false, false, true);
            Kernel.MeshingKernel.SetSize(ov, lcar3);

            Kernel.MeshingKernel.SetSize(ov, lcar2);
